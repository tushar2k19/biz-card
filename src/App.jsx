...
import { Upload, Camera, Trash2, Download, Edit2, Save, X, Search, Grid, List, Mail, Phone, MapPin, Globe, Briefcase, User, Building2, Plus } from 'lucide-react';
...
const BusinessCardScanner = () => {
  ...
  const loadCards = () => {
    ...
    try {
      ...
      console.log('No existing cards found or error loading:', error);
    } finally {
  ...
  const saveCards = (cardsToSave) => {
    try {
      localStorage.setItem('businessCards', JSON.stringify(cardsToSave));
    } catch (error) {
      console.error('Failed to save cards:', error);
      showNotification('Storage limit reached. Please delete some cards.', 'error');
    }
  };
  ...
  const processImage = async (file) => {
    ...
    try {
      ...
      const imageData = await fileToDataUrl(optimizedFile);
      ...
      const extractedData = extractCardData(data);
      ...
      showNotification('Business card scanned successfully!', 'success');
    } catch (error) {
      console.error('OCR Error:', error);
      if (error?.code === 'invalid_card') {
        setInvalidCardMessage(error.message);
      } else {
        showNotification(error.message || 'Failed to process image. Please try again.', 'error');
      }
    } finally {
      setIsScanning(false);
    }
  ...
  const extractCardData = (data) => {
    ...
    if (!jsonMatch) {
      const invalidError = new Error(textContent.text.trim());
      invalidError.code = 'invalid_card';
      throw invalidError;
    }
  ...
  const clearAllData = () => {
    if (window.confirm('Are you sure you want to delete ALL cards? This cannot be undone.')) {
      ...
      setSelectedCard(null);
      setEditingId(null);
      showNotification('All cards deleted', 'success');
    }
  };
      ...
      <div className="relative z-10 container mx-auto px-3 sm:px-4 lg:px-6 py-6 sm:py-8">
        ...
        <div className="bg-white/5 backdrop-blur-2xl rounded-3xl p-4 md:p-6 mb-8 shadow-[0_35px_120px_rgba(0,0,0,0.55)] border border-white/10">
          <div className="flex flex-col gap-4">
            ...
            {/* Backup/Restore Actions - Hidden but functional */}
            {cards.length > 0 && (
              <div className="hidden">
                ...
                <button onClick={exportToJSON}>Backup</button>
        ...
        {cards.length === 0 ? (
          <div className="text-center py-12 sm:py-20">
            ...
            <h3 className="text-xl sm:text-2xl font-semibold text-white mb-2 sm:mb-3">No cards yet</h3>
            <p className="text-white/60 mb-4 sm:mb-6 text-sm sm:text-base px-4">Upload your first business card to get started</p>
            <button
              onClick={() => fileInputRef.current?.click()}
...
const CardItem = ({ card, viewMode, isEditing, isDeleting, onEdit, onDelete, onSave, onCancel, onSelect }) => {
    ...
    return (
      <div className={`bg-white/5 backdrop-blur-2xl rounded-2xl p-4 sm:p-6 shadow-[0_25px_90px_rgba(0,0,0,0.6)] border border-white/10 hover:bg-white/10 transition-all duration-300 cursor-pointer ${isDeleting ? 'deleting-card' : ''}`} onClick={onSelect}>
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6">
          <img src={card.image} alt="Business card" className="w-full sm:w-32 h-32 sm:h-20 object-cover rounded-lg" />
  ...
  return (
    <div className={`bg-white/5 backdrop-blur-2xl rounded-2xl overflow-hidden shadow-[0_30px_110px_rgba(0,0,0,0.6)] border border-white/10 hover:scale-105 transition-all duration-300 cursor-pointer group ${isDeleting ? 'deleting-card' : ''}`} onClick={onSelect}>
      <div className="relative h-40 sm:h-48 overflow-hidden">
        <img src={card.image} alt="Business card" className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
      ...
      <div className="bg-[#0a0f18] rounded-3xl max-w-4xl w-full my-4 sm:my-8 shadow-[0_40px_140px_rgba(0,0,0,0.7)] border border-white/10" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="p-4 sm:p-8 max-h-[calc(90vh-80px)] overflow-y-auto space-y-6">
          <div className="grid md:grid-cols-2 gap-6 sm:gap-8">
            <div>
              <img src={card.image} alt="Business card" className="w-full rounded-2xl shadow-[0_25px_80px_rgba(0,0,0,0.6)] border border-white/10" />
            </div>
